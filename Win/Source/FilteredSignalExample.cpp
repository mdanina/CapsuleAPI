...
std::ofstream artifactsEegStream;
std::ofstream eegQualityStream;
void onSessionEEGData(clCSession, clCEEGTimedData eegData) {
    ...
    const int32_t artifactsByChannelCount = clCEEGTimedData_GetArtifactsByChannelCount(eegData);
    const int32_t eegQuality = clCEEGTimedData_GetEEGQualityCount(eegData);
    std::cout << "Session EEG data received " << channels << " channels and " << samples << " samples and " << artifactsByChannelCount << " artifactsByChannelCount and " << eegQuality << " eegQualityCount" << std::endl;
    ...
    for (int32_t i = 0; i < artifactsByChannelCount; ++i) {
        artifactsEegStream << clCEEGTimedData_GetArtifactsByChannel(eegData, i);
        if (i == artifactsByChannelCount - 1) {
            artifactsEegStream << std::endl;
        } else {
            artifactsEegStream << ',';
        }
    }

    for (int32_t i = 0; i < eegQuality; ++i) {
        eegQualityStream << clCEEGTimedData_GetEEGQuality(eegData, i);
        if (i == eegQuality - 1) {
            eegQualityStream << std::endl;
        } else {
            eegQualityStream << ',';
        }
    }
...
void onSessionStarted(clCSession session) {
    ...
    if (writeCsv) {
        ...
        artifactsEegStream.open("artifacts_eeg.csv");
        eegQualityStream.open("eegQuality.csv");
...
void ClientLoop() {
    ...
    if (artifactsEegStream.is_open()) {
        artifactsEegStream.close();
    }
    if (eegQualityStream.is_open()) {
        eegQualityStream.close();
    }
